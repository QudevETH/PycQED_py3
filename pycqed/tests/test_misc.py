...
class Test_misc(unittest.TestCase):
    ...
    def test_rotate_complex_number(self):
        ...
        np.testing.assert_almost_equal(
            1j * real_vec, rotate_complex(real_vec, 90, deg=True)
        )

    def test_span(self):
        linspan = gen.span_num(3.8, 0.4, 21)
        self.assertAlmostEqual(min(linspan), 3.6)
        self.assertAlmostEqual(max(linspan), 4)
        self.assertAlmostEqual(len(linspan), 21)

        rangespan = gen.span_step(3.8, 0.4, 0.05)
    ...
    def test_gen_sweep_pts(self):
        ...
        linspan = gen.gen_sweep_pts(center=3.8, span=0.2, num=21)
        linspan2 = gen.span_num(3.8, 0.2, 21)
        np.testing.assert_array_equal(linspan, linspan2)

        ran = gen.gen_sweep_pts(start=3.8, stop=4.2, step=0.05)
        np.testing.assert_array_equal(ran, np.arange(3.8, 4.2001, 0.05))

        ran = gen.gen_sweep_pts(center=3.8, span=0.2, step=0.05)
        np.testing.assert_array_equal(ran, gen.span_step(3.8, 0.200, 0.05))
        ...
        with self.assertRaises(ValueError):
            gen.gen_sweep_pts(start=3.8, stop=0.3)
        with self.assertRaises(ValueError):
            gen.gen_sweep_pts(center=3.8, span=0.3)
