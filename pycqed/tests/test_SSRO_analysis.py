...
class Test_SSRO_discrimination_analysis(unittest.TestCase):
    ...
    def test_discrimination_fidelity(self):
        ...
        a = ma.SSRO_discrimination_analysis(
            label="dummy_Butterfly", plot_2D_histograms=False
        )

        self.assertEqual(
            a.folder, os.path.join(self.datadir, "20161214", "120000_dummy_Butterfly")
        )
        mu_a = a.mu_a
        mu_b = a.mu_b
        ...
        self.assertAlmostEqual(
            a.theta % 180, np.angle(a.mu_b - a.mu_a, deg=True), places=1
        )
        diff_v_r = rotate_complex((mu_b - mu_a), -a.theta)
        self.assertAlmostEqual(diff_v_r.imag, 0)

        self.assertAlmostEqual(
            a.opt_I_threshold, np.mean([mu_a.real, mu_b.real]), places=1
        )
    ...
    def test_rotated_discrimination_fidelity(self):
        ...
        a = ma.SSRO_discrimination_analysis(
            label="dummy_Butterfly", theta_in=-a.theta, plot_2D_histograms=True
        )
        self.assertEqual(
            a.folder, os.path.join(self.datadir, "20161214", "120000_dummy_Butterfly")
        )
...
class Test_multiplexed_SSRO_analysis(unittest.TestCase):
    ...
    def test_two_qubit_ssro(self):
        ...
        mu_mat_exp = np.array([[1.04126946, -0.00517882], [-0.03172471, 1.00574731]])
