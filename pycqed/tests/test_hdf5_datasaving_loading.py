...
class Test_HDF5(unittest.TestCase):
    ...
    def test_writing_and_reading_dicts_to_hdf5(self):
        ...
        try:
            ...
            self.assertEqual(
                test_dict["list_of_mixed_type"], new_dict["list_of_mixed_type"]
            )
            self.assertEqual(
                test_dict["list_of_mixed_type"][0], new_dict["list_of_mixed_type"][0]
            )
            self.assertEqual(
                test_dict["list_of_mixed_type"][2], new_dict["list_of_mixed_type"][2]
            )

            self.assertEqual(
                test_dict["tuple_of_mixed_type"], new_dict["tuple_of_mixed_type"]
            )
            self.assertEqual(
                type(test_dict["tuple_of_mixed_type"]),
                type(new_dict["tuple_of_mixed_type"]),
            )
            self.assertEqual(
                test_dict["tuple_of_mixed_type"][0], new_dict["tuple_of_mixed_type"][0]
            )
            self.assertEqual(
                test_dict["tuple_of_mixed_type"][2], new_dict["tuple_of_mixed_type"][2]
            )
            ...
            self.assertEqual(
                test_dict["a list of strings"], new_dict["a list of strings"]
            )
            self.assertEqual(
    ...
    def test_loading_settings_onto_instrument(self):
        ...
        gen.load_settings_onto_instrument_v2(
            self.mock_parabola, label="test_MC_snapshot_storing"
        )
