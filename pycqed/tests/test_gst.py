...
class Test_GST_CC(unittest.TestCase):
    ...
    def test_parser(self):
        testfile = os.path.join(
            pq.__path__[0], "tests", "gst_files", "GST_template_short.txt"
        )
        gateDict = {"Gi": "I 20", "Gx": "X90 QR", "Gy": "Y90 QR", "RO": "RO QR"}

        exp_list = gstCC.get_experiments_from_file(
            testfile, gateDict, use_pygsti_parser=True
        )
        ...
        exp_list = gstCC.get_experiments_from_file(
            testfile, gateDict, use_pygsti_parser=False
        )
