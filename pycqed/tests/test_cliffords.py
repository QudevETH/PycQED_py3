...
from pycqed.measurement.randomized_benchmarking.clifford_decompositions import (
    gate_decomposition,
    epstein_fixed_length_decomposition,
)

from pycqed.measurement.randomized_benchmarking import two_qubit_clifford_group as tqc
from pycqed.measurement.randomized_benchmarking.generate_clifford_hash_tables import (
    construct_clifford_lookuptable,
)
...
class TestHashedLookuptables(TestCase):
    def test_single_qubit_hashtable_constructed(self):
        hash_table = construct_clifford_lookuptable(
            tqc.SingleQubitClifford, np.arange(24)
        )
        for i in range(24):
    ...
    def test_two_qubit_hashtable_constructed(self):
        hash_table = construct_clifford_lookuptable(
            tqc.TwoQubitClifford, np.arange(11520)
        )
...
class TestCliffordCalculus(TestCase):
    ...
    def test_products(self):
        ...
        target_hash = crc32(
            tqc.TwoQubitClifford(0).pauli_transfer_matrix.round().astype(int)
        )
    ...
    def test_product_order(self):
        ...
        dot_prod = np.dot(Cl_528.pauli_transfer_matrix, Cl_9230.pauli_transfer_matrix)
        np.testing.assert_array_equal(Cliff_prod.pauli_transfer_matrix, dot_prod)
...
class TestCliffordClassRBSeqs(TestCase):
    ...
    def test_interleaved_randomized_benchmarking_sequence_1Q(self):
        ...
        for seed, net_cl in zip(seeds, net_cliffs):
            intl_cliffords = rb.randomized_benchmarking_sequence(
                n_cl=20, desired_net_cl=0, number_of_qubits=1, seed=0, interleaving_cl=0
            )
            ...
            new_cliff[1::2] = 0
            np.testing.assert_array_equal(intl_cliffords, new_cliff)
    ...
    def test_interleaved_randomized_benchmarking_sequence_2Q(self):
        ...
        for seed, net_cl in zip(seeds, net_cliffs):
            intl_cliffords = rb.randomized_benchmarking_sequence(
                n_cl=20, desired_net_cl=0, number_of_qubits=2, seed=0, interleaving_cl=0
            )
            cliffords = rb.randomized_benchmarking_sequence(
