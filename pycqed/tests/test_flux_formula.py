...
class Test_Flux_formula(unittest.TestCase):
    ...
    def test_freq_to_dac(self):
        ...
        dac_sweet_spot = 0.02
        E_c = 300e6
        dac_flux_coefficient = 1 / 0.2
        for asym in [0, 0.5, 0.9]:
            dac_voltages = np.linspace(-0.100, 0.100, 201)
            ...
            self.assertAlmostEqual(dac_voltages[np.argmax(freqs)], dac_sweet_spot)

            # Simple model excluding assymetry
            simple_freq_model = (f_max + E_c) * np.sqrt(
                abs(np.cos(dac_flux_coefficient * (dac_voltages - dac_sweet_spot)))
            ) - E_c
    ...
    def test_dac_to_freq(self):
        ...
        np.testing.assert_array_almost_equal(
            dac_voltages_neg_branch, recovered_dac_voltages
        )
