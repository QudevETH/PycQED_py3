...
class Test_SSRO_auto_angle(unittest.TestCase):
    ...
    def test_angles(self):
        ...
        for angle in np.arange(0, 360, 30):
            ...
            aut = ma.Singleshot_Readout_Analysis(
                t_start=ts,
                t_stop=te,
                label=label,
                extract_only=True,
            ...
            opt = ma.Singleshot_Readout_Analysis(
                t_start=ts,
                t_stop=te,
                label=label,
                extract_only=True,
...
class Test_SSRO_discrimination_analysis(unittest.TestCase):
    ...
    def test_SSRO_analysis_basic_1D(self):
        ...
        t_stop = t_start
        a = ma.Singleshot_Readout_Analysis(
            t_start=t_start, t_stop=t_stop, options_dict={"plot_init": True}
        )
        np.testing.assert_almost_equal(
            a.proc_data_dict["threshold_raw"], -3.66, decimal=2
        )
        np.testing.assert_almost_equal(
            a.proc_data_dict["F_assignment_raw"], 0.922, decimal=3
        )
        self.assertBetween(a.proc_data_dict["threshold_fit"], -3.69, -3.62)
        np.testing.assert_almost_equal(
            a.proc_data_dict["F_assignment_fit"], 0.920, decimal=2
        )
        np.testing.assert_almost_equal(
            a.proc_data_dict["threshold_discr"], -3.64, decimal=1
        )
        np.testing.assert_almost_equal(a.proc_data_dict["F_discr"], 0.996, decimal=2)
    ...
    def test_SSRO_analysis_basic_1D_wrong_peak_selected(self):
        ...
        t_stop = t_start
        a = ma.Singleshot_Readout_Analysis(
            t_start=t_start, t_stop=t_stop, extract_only=True
        )
        self.assertBetween(a.proc_data_dict["threshold_raw"], -3.3, -3.2)
        np.testing.assert_almost_equal(
            a.proc_data_dict["F_assignment_raw"], 0.944, decimal=2
        )
        self.assertBetween(a.proc_data_dict["threshold_fit"], -3.3, -3.2)
        np.testing.assert_almost_equal(
            a.proc_data_dict["F_assignment_fit"], 0.944, decimal=2
        )
        self.assertBetween(a.proc_data_dict["threshold_discr"], -3.3, -3.2)
        np.testing.assert_almost_equal(a.proc_data_dict["F_discr"], 0.99, decimal=2)
    ...
    def test_SSRO_analysis_basic_1D_misfit(self):
        ...
        np.testing.assert_almost_equal(
            a.proc_data_dict["F_assignment_raw"], 0.949, decimal=2
        )
        self.assertBetween(a.proc_data_dict["threshold_fit"], -1, -0.7)
        np.testing.assert_almost_equal(
            a.proc_data_dict["F_assignment_fit"], 0.945, decimal=2
        )
        self.assertBetween(a.proc_data_dict["threshold_discr"], -1, -0.7)
        np.testing.assert_almost_equal(a.proc_data_dict["F_discr"], 1.000, decimal=2)
        self.assertLess(a.proc_data_dict["residual_excitation"], 0.09)
        np.testing.assert_almost_equal(
            a.proc_data_dict["measurement_induced_relaxation"], 0.1, decimal=1
        )
...
class Test_readout_analysis_functions(unittest.TestCase):
    ...
    def test_get_assignment_fid_from_cumhist(self):
        chist_0 = np.array([0, 0, 0, 0, 0, 0.012, 0.068, 0.22, 0.43, 0.59, 0.78, 1, 1])
        chist_1 = np.array(
            [0, 0.01, 0.05, 0.16, 0.21, 0.24, 0.38, 0.62, 0.81, 1, 1, 1, 1]
        )
        centers = np.linspace(0, 1, len(chist_0))

        # Fidelity for identical distributions should be 0.5 (0.5 is random)
        fid, th = ra.get_assignement_fid_from_cumhist(chist_0, chist_0, centers)
        self.assertEqual(fid, 0.5)

        # Test on the fake distribution
        fid, threshold = ra.get_assignement_fid_from_cumhist(chist_0, chist_1)
        np.testing.assert_almost_equal(fid, 0.705)
        np.testing.assert_almost_equal(threshold, 9)
        # Test on the fake distribution
        fid, threshold = ra.get_assignement_fid_from_cumhist(chist_0, chist_1, centers)
        np.testing.assert_almost_equal(fid, 0.705)
...
class Test_multiplexed_readout_analysis(unittest.TestCase):
    ...
    def test_multiplexed_readout_analysis(self):
        ...
        t_stop = t_start
        a = ma.Multiplexed_Readout_Analysis(
            t_start=t_start, t_stop=t_stop, qubit_names=["QR", "QL"]
        )
        np.testing.assert_almost_equal(
            a.proc_data_dict["F_ass_raw QL"], 0.72235812133072408
        )

        np.testing.assert_almost_equal(
            a.proc_data_dict["F_ass_raw QR"], 0.81329500978473579
        )

        np.testing.assert_almost_equal(
            a.proc_data_dict["threshold_raw QL"], 1.9708007812500004
        )
        np.testing.assert_almost_equal(
            a.proc_data_dict["threshold_raw QR"], -7.1367667055130006
        )
