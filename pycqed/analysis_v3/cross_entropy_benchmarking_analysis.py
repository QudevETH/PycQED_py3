...
def get_two_qb_gate_length(timestamp, chip_name, gate_name='CZ_nztc'):
    mobjn = hlp_mod.get_param_from_metadata_group(timestamp, 'meas_objs')
    params_dict = {f'pulse_length':
                       f'Instrument settings.{chip_name}.{gate_name}_'
                       f'{mobjn[0]}_{mobjn[1]}_pulse_length'}
    dd = hlp_mod.get_params_from_hdf_file({}, params_dict,
                                          folder=a_tools.get_folder(timestamp))
    if dd['pulse_length'] == 0:
        params_dict = {f'pulse_length':
                           f'Instrument settings.{chip_name}.{gate_name}_'
                           f'{mobjn[1]}_{mobjn[0]}_pulse_length'}
...
def simulate_circuits_2qbs(nr_cycles, nr_seq, circuits_list, T1, T2, t_gate,
    ...
    for i in range(nr_seq):
        rho = rho0
        # print([idx for idx, g in enumerate(circuits_list[i])
        #        if len(g) == 2 + (nr_cycles+1)*3])
...
def transfer(sp):
    lis = []
    for circuit_lis in sp.get_sweep_params_property('values', 1):
