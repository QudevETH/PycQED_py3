...
xentropy = lambda pops0, pops1: 2*np.sum([p0*p1
...
sqrtX2q = qt.qip.operations.gates.rx(np.pi/2, N=None, target=0)
sqrtY2q = qt.qip.operations.gates.ry(np.pi/2, N=None, target=0)
sqrtW = (qt.qip.operations.gates.rz(np.pi/4, N=None, target=0) *
         qt.qip.operations.gates.rx(np.pi/2, N=None, target=0)*
         qt.qip.operations.gates.rz(-np.pi/4, N=None, target=0))
T = qt.qip.operations.gates.rz(np.pi/4, N=None, target=0)
...
U1 = lambda da, df, t: sp.linalg.expm(
    -1j*((df*2*np.pi)*e*e.conj().T + (np.pi/2/t + da/t)*qt.sigmay()/2).full()*t)
...
def time_evolve_gate_prop(angle, T1, T2s, t_gate, delta_ang=0, delta_f=0):
...
def simulate_circuits_1qb(nr_cycles, nr_seq, rots_qt, T1, T2,
                          t_gate, delta_ang=0, delta_f=0):
    t0 = time.time()
    # get pops_sim
    pops_sim = np.zeros((nr_seq, 2))
    for i in range(nr_seq):
        rho = sqrtYqt*qt.thermal_dm(2, 0)*sqrtYqt.dag()
        Uy = time_evolve_gate_prop(np.pi/2, T1, T2, t_gate=t_gate,
                                   delta_ang=delta_ang, delta_f=delta_f)
...
def calculate_ideal_circuit_1qb(nr_cycles, nr_seq, rots, t_gate=None,
    ...
    :param init_state: numpy array of initial state
...
#             pops_sim[i],
#             (np.abs(np.matmul(reduce(np.matmul, Us), psi))**2).T[0])
...
def avg_loss(ctrl_errs, grad):
    ...
    cost_values = np.zeros(nr_seq)
    for i in range(nr_seq):
        pops_ideal = xeb_ana.calculate_ideal_circuit_1qb(nr_cycles, 1, [z_rots[i]], t, Uvar)[0]
        #         print()
        #         print(pops_sim[i])
        #         print(pops_ideal)
        cost_values[i] = xeb_ana.xentropy(pops_sim[i], pops_ideal)
    return -np.mean(cost_values)
...
def avg_loss_fidelity(ctrl_errs, grad):
    ...
    freq_det = xeb_ana.convert_to_mhz(ctrl_errs[1], t)
    Uvar = xeb_ana.U1(ampl_angle, freq_det, t)
    pops_ideal = xeb_ana.calculate_ideal_circuit_1qb(nr_cycles, nr_seq, z_rots, t, Uvar)
    #     print()
...
def time_evolve_gate_prop_2qbs(T1, T2, t_gate, phid1=0, phid2=0, delta_phic=0, phis=0, phisa=0, nr_qubits=2):
    #     t_gate = 0.1 # us
    ...
    H = qt.Qobj(2*np.pi*delta1*np.kron(e @ e.conj().T, eye) + \
                ...
                Js*(np.exp(1j*phisa/2)*np.kron(g, e)@np.kron(e, g).conj().T + np.exp(-1j*phisa/2)*np.kron(e, g)@np.kron(g, e).conj().T)+ \
...
def calculate_fidelities_purities_1qb(data_dict, data_key='correct_readout',
    ...
    for mobjn in meas_obj_names:
        pops_meas_all = data_dict[mobjn][data_key]['pe'].reshape(
...
def transfer(sp):
    lis = []
    for circuit_lis in list(sp[1].values())[0][0]:
        # print(circuit_lis)
        #circuit_lis = list(sp[1].values())[0][0][0]
...
def calculate_fidelities_purities_2qb(data_dict, data_key='correct_readout',
    ...
    while i < len(cycles):
        ...
        purity[cycles[i]] = p_m
        prop_idea = proba_from_all_circuits(current_circuits)
        xeb = xentropy_fidelity(pops_meas, np.array(prop_idea), d)
        ...
        for mobjn in fidelities:
            ...
            if plot:
                ...
                if fit:
                    ...
                    ctrl_err_err = 0.5*np.sqrt(epc_err**2 + ppc_err**2)
...
def plot_porter_thomas_dist(data_dict, nr_bins=50, data_key='correct_readout',
                            ...
                            figure_height=4.1, filename=None,
                            numcols=None, numrows=None, **params):
    try:
        d = hlp_mod.get_param('dim_hilbert', data_dict, raise_error=True,
                              **params)
        print('d', d)
            ...
            for i, nrc in enumerate(cycles):
                ...
                if isinstance(proba_exp_all, dict):
                    ...
                    proba_exp = np.reshape(proba_exp[:, i],
                                           (len(proba_exp[:, i]), 1))
                    proba_exp = np.concatenate([1-proba_exp, proba_exp], axis=1)
                else:
                    proba_exp = np.concatenate([proba_exp_all[:, :2],
                                                proba_exp_all[:, 3:5]], axis=1)
                    proba_exp = proba_exp.reshape(swpts.length(1),
                                                  swpts.length(0),
                                                  proba_exp.shape[-1])
                ...
                if i == 0:
                    ax.legend(frameon=False, loc='lower center',
                              # bbox_to_anchor=(0.95, 0.925), ncol=2)
                              bbox_to_anchor=(0.5, 0.925), ncol=2)

            fig.text(0, 0.5, r'Normalized counts', ha='left', va='center',
                     rotation=90)
            fig.text(0.5, 0, f'Probability$\\cdot {d}$', va='bottom',
                     ha='center')
            fig.text(0.5, 1, f'XEB {mobjn} - {timestamp}',
                     ha='center', va='top')
            fig.patch.set_color('0.9')
            fig.subplots_adjust(0.11, 0.075, 0.99, 0.92, wspace=0.05)
...
def calculate_optimal_nr_cycles(data_dict, idx0=1, joint_processing=False,
                                fidelities=None, exlude_from_plot=False,
                                plot=True, savefig=True, fmts=None,
