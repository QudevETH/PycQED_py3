    ...
    for key in plot_dicts_keys:
        ...
        if axes_pdict['fig_id'] not in axs:
            ...
            if axes_pdict.get('3d', False):
                ...
                axs[axes_pdict['fig_id']] = Axes3D(
                    figs[axes_pdict['fig_id']],
                    azim=axes_pdict.get('3d_azim', -35),
                    elev=axes_pdict.get('3d_elev', 35))
                axs[axes_pdict['fig_id']].patch.set_alpha(0)

            # transparent background around axes for presenting data
            axes_pdict['transparent_bg'] = axes_pdict.get('transparent_bg', True)
            if axes_pdict['transparent_bg']:
                axs[axes_pdict['fig_id']].patch.set_alpha(0)
