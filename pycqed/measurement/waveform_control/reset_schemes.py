...
class ResetScheme(InstrumentModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # FIXME: better name?
        self.add_parameter(
            "repetition_buffer_start",
            label="repetition_buffer_start",
            unit="s",
            initial_value=0,
            vals=validators.Numbers(),
            parameter_class=ManualParameter,
        ...
        self.add_parameter(
            "repetition_buffer_end",
            label="repetition_buffer_end",
            unit="s",
            initial_value=0,
            vals=validators.Numbers(),
            parameter_class=ManualParameter,
        ...
        self.instr_ref = (
            self.root_instrument if ref_instrument is None else ref_instrument
        )
    ...
    def add_operation(self, operation_name, init_values=None):
        ...
        if operation_name not in self.instr_ref.operations():
            raise ValueError(
                f"Operation name {operation_name} unknown to "
        ...
        # add operation
        self._operations[operation_name] = deepcopy(
            self.instr_ref._operations[operation_name]
        )
        ...
        for pulse_param_name, param_name in self._operations[operation_name].items():
            if self.instr_ref.parameters[param_name].vals is not None:
                vals = validators.MultiType(
                    self.instr_ref.parameters[param_name].vals,
            ...
            self.add_parameter(
                param_name,
                initial_value=init_values.get(param_name, self.DEFAULT_VALUE),
                vals=vals,
                parameter_class=ManualParameter,
            )
    ...
    def reset_block(self, name=None, sweep_params=None, **block_kwargs):
        ...
        for i in range(self.repetitions()):
            init_i = self._reset_block(
                name + f"_{i}", sweep_params, **block_kwargs
            ).build(
    ...
    def get_operation_dict(self, operation_dict=None):
        ...
        for op in init_specific_params:
            operation_dict[self.get_opcode(op)] = deepcopy(
                ref_instr_op_dict[op + f" {self.instr_ref.name}"]
            )
            operation_dict[self.get_opcode(op)].update(init_specific_params[op])
            operation_dict[self.get_opcode(op)].update({"op_code": self.get_opcode(op)})
    ...
    def _get_operation_dict(self, operation_dict=None):
        ...
        for op_name, op in self.operations().items():
            ...
            for argument_name, parameter_name in op.items():
                operation_dict[op_code][argument_name] = self.get(parameter_name)
        return operation_dict
    ...
    def get_init_specific_params(self, operations=None):
        ...
        for op in operations:
            params[op] = {
                k: v
                for k, v in self._get_operation_dict()[self.get_opcode(op)].items()
...
class Preselection(ResetScheme):
    ...
    def __init__(self, parent, **kwargs):
        ...
        """
        super().__init__(parent, name="preselection", operations=("RO", "FP"), **kwargs)

        self.add_parameter(
            "compensate_ro_flux",
            label="compensate_ro_flux",
            initial_value=False,
    ...
    def _reset_block(self, name, sweep_params, **kwargs):
        ...
        if (
            preselection_ro["pulse_type"] == "GaussFilteredCosIQPulseWithFlux"
            and self.compensate_ro_flux()
        ):
            compensation_fp = deepcopy(op_dict[f"FP {self.instr_ref.name}"])
            compensation_fp["pulse_type"] = "BufferedSquarePulse"
            compensation_fp["gaussian_filter_sigma"] = preselection_ro[
                "flux_gaussian_filter_sigma"
            ]
        ...
        # additional changes
        return block_mod.Block(name, presel_pulses, copy_pulses=False, **kwargs)
...
class FeedbackReset(ResetScheme):
    ...
    def get_operation_dict(self, operation_dict=None):
        ...
        operation_dict[self.get_opcode("I")] = deepcopy(
            operation_dict[self.get_opcode("X180")]
        )
    ...
    def _validate_ro_feedback_delay(self, ro_feedback_delay):
        ...
        if hasattr(self.instr_ref.inst_acq.get_instr(), "feedback_latency"):
            ...
            log.warning(
                f"{self.instr_ref.get_instr()} has no function \
            feedback_latency()"
            )
    ...
    def _validate_codeword_state_map(self, codeword_state_map):
        ...
        if len(codeword_state_map) % 2 != 0:
            msg = (
                f"codeword_state_map must have even number of codewords"
...
class ParametricFluxReset(ResetScheme):
    ...
    def __init__(self, parent, operations=None, **kwargs):
        ...
        if operations is None:
            ...
            if len(operations) == 0:
                raise ValueError(
                    "No operation found starting with 'PFM'"
                    f" in the root instrument "
                    f"{parent.root_instrument}."
                )

        super().__init__(
            parent, name=self.DEFAULT_INSTANCE_NAME, operations=operations, **kwargs
        )
    ...
    def _reset_block(self, name, sweep_params, **kwargs):
        ...
        reset_pulses = [
            deepcopy(op_dict[self.get_opcode(op)]) for op in self.operations()
        ]
